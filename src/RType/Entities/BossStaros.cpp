...
namespace rtype::entities {
    BossStaros::BossStaros(ecs::ComponentManager &componentManager, ecs::EntityManager &entityManager, components::Position
    pos, components::NetId netId) : AEntity(entityManager){
        ...
        move.emplace_back(components::Velocity({0.2, -1.0, 0}));
        componentManager.addComponent<components::IA>(_id, {move, models::BOSS_STAROS, 0.6, 1.2}, entityManager);
#ifdef RTYPE_IS_CLIENT
        components::Sprite sprite;
        sprite.priority = { 1 };

        sprite.sprite = std::make_shared<sf::Sprite>();
        sprite.texture = TextureManager::getInstance().getTexture("boss_staros");
...
#else
        componentManager.addComponent<components::AiShoot>(_id, {0.4f}, entityManager);
#endif
    }
