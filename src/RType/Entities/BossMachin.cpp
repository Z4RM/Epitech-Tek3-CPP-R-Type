...
namespace rtype::entities {
        BossMachin::BossMachin(ecs::ComponentManager &componentManager, ecs::EntityManager &entityManager, components::Position
        pos, components::NetId netId) : AEntity(entityManager){
                components::Size size = {64, 80};
                ...
                componentManager.addComponent<components::Speed>(_id, { 120 }, entityManager);
                ...
                move.emplace_back(components::Velocity({0.2, -1.0, 0}));
                move.emplace_back(components::Velocity({1.0, 0, 0}));
                move.emplace_back(components::Velocity({-1.0, 0, 0}));
                move.emplace_back(components::Velocity({0.2, 0.7, 0}));
                move.emplace_back(components::Velocity({-0.2, -0.7, 0}));
                componentManager.addComponent<components::IA>(_id, {move, models::BOSS_MACHIN, 0.2, 0.8}, entityManager);
#ifdef RTYPE_IS_CLIENT
                components::Sprite sprite;
                sprite.priority = { 1 };

                sprite.sprite = std::make_shared<sf::Sprite>();
                sprite.texture = TextureManager::getInstance().getTexture("boss_machin");
...
#else
                componentManager.addComponent<components::AiShoot>(_id, {0.2f}, entityManager);
#endif
        }
}
