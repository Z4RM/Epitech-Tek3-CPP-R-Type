...
namespace rtype::entities {
    BossRobot::BossRobot(ecs::ComponentManager &componentManager, ecs::EntityManager &entityManager, components::Position
    pos, components::NetId netId) : AEntity(entityManager){
        components::Size size = {176, 144};
        ...
        move.emplace_back(components::Velocity({-0.3, 1.0, 0}));
        move.emplace_back(components::Velocity({0.3, -1.0, 0}));
        move.emplace_back(components::Velocity({2.0, 0, 0}));
        move.emplace_back(components::Velocity({-2.0, 0, 0}));
        componentManager.addComponent<components::IA>(_id, {move, models::BOSS_ROBOT, 0.3, 1.2}, entityManager);
#ifdef RTYPE_IS_CLIENT
        components::Sprite sprite;
        sprite.priority = { 1 };

        sprite.sprite = std::make_shared<sf::Sprite>();
        sprite.texture = TextureManager::getInstance().getTexture("boss_robot");
