...
namespace rtype::entities {
    BossCask::BossCask(ecs::ComponentManager &componentManager, ecs::EntityManager &entityManager, components::Position
    pos, components::NetId netId) : AEntity(entityManager){
        ...
        move.emplace_back(components::Velocity({-1.0, 0, 0}));
        componentManager.addComponent<components::IA>(_id, {move, models::BOSS_CASK, 0.5, 0.8}, entityManager);
#ifdef RTYPE_IS_CLIENT
        components::Sprite sprite;
        sprite.priority = { 1 };

        sprite.sprite = std::make_shared<sf::Sprite>();
        sprite.texture = TextureManager::getInstance().getTexture("boss_cask");
